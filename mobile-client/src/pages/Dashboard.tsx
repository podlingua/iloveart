...
import { extractSource } from "@/lib/api/client";
...
export default function Dashboard() {
  ...
  const startRandom = () => navigate(buildSessionUrl());

  const startCustom = () => {
    if (!topic.trim()) return;
    navigate(buildSessionUrl({ topic: topic.trim() }));
  };
  ...
  const handleFileSelected = async (file: File) => {
    ...
    setUploadError(null);
    try {
      const source = await extractSource(file, lang, session?.access_token ?? null);
      saveSource(source);
      navigate(buildSessionUrl({ source: "1" }));
    } catch (err) {
